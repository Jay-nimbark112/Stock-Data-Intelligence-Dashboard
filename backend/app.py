...
db_path = os.path.join(basedir, 'tickerpulse.db')
...
@app.route("/")
def home():
    return jsonify({"message": "🚀 Stock Data API is running on Render!"})
...
def process_stock_data(symbol, days=30):
    try:
        ...
        df["Market_Correlation"] = df["Daily_Return"].rolling(window=60).corr(
            df["Market_Close"].pct_change()
        )
...
def get_summary(symbol):
    ...
    # Save to DB
    try:
        log = SearchLog(symbol=symbol, price=current_price)
        db.session.add(log)
        db.session.commit()
    except Exception as e:
        print(f"DB Error: {e}")
